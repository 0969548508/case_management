...
@include('navbar')
    <div class="d-sm-flex align-items-center justify-content-between mb-20">
        <h1 class="title mb-0">Deleted Clients</h1>
    </div>
                ...
                @foreach ($listClients as $client)
                    @if ($client->in_trash)
                        <tr>
                            ...
                            <td class="data-{{$client->id}}">
                                <button type="button" class="btn btn-restore" id="btn-restore" onclick="showModalRestore('{{ route('restoreClient', $client->id) }}')">
                                    Restore
                                </button>
                                @can('delete clients and client locations')
                                    @if (DB::table('cases')->where('client_id', $client->id)->count() == 0)
                                        <button type="button" class="btn btn-delete-permanently" id="btn-delete-permanently" onclick="showModalDelete('{{ route('deletePermanentlyClient', $client->id) }}')">
                                            Delete Permanently
                                        </button>
                                    @endif
                                @endcan
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="deleteTitle">{{ __('Delete Client Permanently') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    {{ __('This action can not be undone. Are you sure you want to delete the client permanently?') }}
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content edit-add-info">
                <div class="modal-header edit-add-info-header">
                    <h5 class="modal-title" id="restoreTitle">{{ __('Restore Deleted Client') }}</h5>
                    <button type="button" class="close btn-x-modal" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    {{ __('Are you sure you want to restore the client?') }}
                </div>
