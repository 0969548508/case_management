@switch($slug)
    ...
    @case('user-create')
    @case('user-update')
        <a class="dropdown-item page-notification-item @if (empty($notification->read_at)) page-notification-unread @endif" href="@if (empty($notifyInfo->url)) javascript:void(0); @else {{ route('readUserNotification', [
            'id'     => $notification->id,
            'url'    => $notifyInfo->url,
            'userId' => $info->id
        ]) }} @endif">
            ...
            @endif
            <div class="page-notification-title">
