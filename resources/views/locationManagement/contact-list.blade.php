    ...
    <div class="col-lg-8 col-sm-12 col-md-12">
        <ul class="nav nav-tabs" role="tablist">
            ...
            @can('view client location contacts')
                <li class="nav-item">
                    <a class="nav-link {{ $activeTab ? 'active':''}}" href="{{ route('showContactListLocation', $locationId) }}">Contacts List</a>
                </li>
            @endcan
            @can('view agreements')
                <li class="nav-item">
            ...
            <div class="card-body tab-content recent-cases">
                <div class="row mb-4">
                    <div class="col-lg-12 col-sm-12 col-md-12 d-sm-flex align-items-center justify-content-between">
                        ...
                        @can('create client location contacts')
                            <a href="#"><button type="button" class="d-none d-sm-inline-block btn btn-create-style" id="btn-add-contact">ADD CONTACT</button></a>
                        @endcan
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-lg-12 col-sm-12 col-md-12">
                        <div class="row ml-0">
                            @can('delete client location contacts')
                                ...
                                @foreach($contactList as $key => $contact)
                                    <tr id="input-data-{{ $contact['id'] }}">
                                        ...
                                        <td class="custom-color-location">
                                            @can('edit client location contacts')
...
<!-- modal add contact -->
<div class="modal fade" id="modal-add-contact" tabindex="-1" role="dialog" aria-labelledby="addTitle" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-dialog-centered">
        <form method="POST" id="add-contact-form" action="{{ route('createContactListLocation', $locationId) }}">
            @csrf
            <div class="modal-content edit-add-info">
                <div class="modal-header edit-add-info-header">
                    <h5 class="modal-title" id="addTitle">{{ __('Add Contact') }}</h5>
                ...
                <div class="modal-body">
                    <div class="form-group row mt-2">
                        <label for="name" class="col-md-12 col-lg-3 col-form-label text-md-left">{{ __('Name *') }}</label>

                        <div class="col-md-12 col-lg-9">
                            <input id="name" type="text" class="form-control" name="name" value="{{ old('name') }}" required autocomplete="name" autofocus placeholder="Input name" pattern=".*\S+.*">
                        </div>
                    </div>

                    <div class="form-group row">
                        <label for="job_title" class="col-md-12 col-lg-3 col-form-label">{{ __('Job Title *') }}</label>

                        <div class="col-md-12 col-lg-9">
                            <input id="job_title" type="text" class="form-control" name="job_title" value="{{ old('job_title') }}" required autocomplete="job_title" placeholder="Input job title" pattern=".*\S+.*">
                        </div>
                    </div>

                    <div class="form-group row">
                        <label for="email" class="col-md-12 col-lg-3 col-form-label text-md-left">{{ __('Email *') }}</label>

                        <div class="col-md-12 col-lg-9">
                            <input id="email" type="email" class="form-control" name="email" value="{{ old('email') }}" required autocomplete="email" placeholder="Enter Email">
                        </div>
                    </div>

                    <div class="form-group row">
                        <label for="phone" class="col-md-12 col-lg-3 col-form-label text-md-left">{{ __('Phone *') }}</label>

                        <div class="col-md-12 col-lg-9">
                            <input id="phone" type="text" class="form-control" name="phone" value="{{ old('phone') }}" required autocomplete="phone" placeholder="Input phone" pattern="\d+" oninput="this.value = this.value.replace(/[^0-9]/g, '').split(/\./).slice(0, 2).join('.')">
                        </div>
                    </div>

                    <div class="form-group row">
                        <label for="mobile" class="col-md-12 col-lg-3 col-form-label text-md-left">{{ __('Mobile') }}</label>

                        <div class="col-md-12 col-lg-9">
                            <input id="mobile" type="text" class="form-control" name="mobile" value="{{ old('mobile') }}" autocomplete="mobile" placeholder="Input mobile" pattern="\d+" oninput="this.value = this.value.replace(/[^0-9]/g, '').split(/\./).slice(0, 2).join('.')">
                        </div>
                    </div>

                    <div class="form-group row">
                        <label for="fax" class="col-md-12 col-lg-3 col-form-label text-md-left">{{ __('Fax') }}</label>

                        <div class="col-md-12 col-lg-9">
                            <input id="fax" type="text" class="form-control" name="fax" value="{{ old('fax') }}" autocomplete="fax" placeholder="Input fax">
                        </div>
                    </div>

                    <div class="form-group row">
                        <label for="note" class="col-md-12 col-lg-3 col-form-label text-md-left">{{ __('Note') }}</label>

                        <div class="col-md-12 col-lg-9">
                            <textarea rows="3" style="width:100%;" class="form-control" name="note"></textarea>
                        </div>
                    </div>

                    <!-- hidden get client id -->
                    <input id="client_id" type="text" class="form-control" name="client_id" value="{{ $clientDetail['id'] }}" hidden>
                </div>
                <div class="modal-footer edit-add-info-footer">
                    <div class="form-group">
                        ...
                        <button type="submit" class="btn btn-add-modal" id="btn-add-contact-modal">{{ __('SAVE') }}</button>
                    </div>
                </div>
            </div>
        </form>
...
<!-- modal edit contact -->
<div class="modal fade" id="modal-edit-contact" tabindex="-1" role="dialog" aria-labelledby="editTitle" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-dialog-centered">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('#btn-delete-contact-modal').click(function() {
                var url = "{{ route('deleteContact', ['locationId'=>$locationDetail['id']]) }}";
